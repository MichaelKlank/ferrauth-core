...
(`cargo-husky` treats any **non-empty** value as “skip”; `true` and `1` are both fine.)

**rust-analyzer / IDE `cargo check`:** The repo’s **`.vscode/settings.json`** sets `CARGO_HUSKY_DONT_INSTALL_HOOKS` to `true` for rust-analyzer (same effect as the `export` above) so `cargo-husky`’s build script does not validate `.cargo-husky/hooks` during analysis (which would otherwise fail if the template script is not executable after checkout, e.g. `chmod +x .cargo-husky/hooks/pre-commit`). Hooks are still installed when you run `cargo test` (or similar) from a normal shell **without** that variable.

To **refresh** hooks after upgrading `cargo-husky`, remove `.git/hooks/pre-commit` (if it was generated by cargo-husky) and run `cargo test --all-features --locked` again (or any command that compiles dev-dependencies). On Unix, ensure `.cargo-husky/hooks/pre-commit` stays **executable** (`chmod +x`).
...
# --locked matches CI and avoids silently changing Cargo.lock
cargo test --all-features --locked

# Run linting
cargo clippy --all-targets --all-features --locked -- -D warnings

# Check formatting (same as pre-commit hook and CI)
cargo fmt --all -- --check
